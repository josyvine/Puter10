...
import android.webkit.PermissionRequest; // Added for WebRTC permission handling
...
import android.content.ClipData; // Added for standard file picker support
 ...
 * WebRTC PERMISSION RESOLUTION: Embedded robust inner MyWebChromeClient with onPermissionRequest support.
 * TTS CONTROLLER: Added native stopSpeaking() callback mapped to web-bridge commands.
 */
public class MainActivity extends AppCompatActivity implements TextToSpeech.OnInitListener {
    ...
    private MyWebChromeClient myWebChromeClient; // Custom client for WebRTC permissions and file pickers
    ...
    private Button btnBrowserScraper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnBrowserScraper = findViewById(R.id.btnBrowserScraper);
        ...
        // Handle clicking the Native Scraping FAB
        ...
        // Custom client configuration containing WebRTC dynamic permission triggers
    ...
    /**
     * Custom WebChromeClient implementation supporting standard file upload dialogs
     * and dynamic runtime WebRTC audio capture permissions inside the WebView.
     */
    private class MyWebChromeClient extends WebChromeClient {
        ...
        public void onPermissionRequest(final PermissionRequest request) {
            Log.d("MainActivity", "onPermissionRequest: WebView requesting resources: " + java.util.Arrays.toString(request.getResources()));
            runOnUiThread(() -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    // Instantly authorize WebView access to standard WebRTC recording and audio streams
                    request.grant(request.getResources());
                    Log.d("MainActivity", "onPermissionRequest: Successfully granted WebRTC permissions to WebView.");
                }
            });
        }
    ...
    /**
     * Safely halts system-level vocalizations immediately to prevent queue stalls.
     */
    public void stopSpeaking() {
        if (isTtsInitialized && tts != null) {
            Log.d("MainActivity", "Halt vocalization requested natively.");
            ActionReportLogger.logAction("TTS_STOP", "Native stop command executed.");
            tts.stop();
        } else {
            Log.w("MainActivity", "Native stop aborted: TTS Engine is not initialized.");
        }
    }

    /**
     * Executes the native scraping sequence.
     */
    ...
    /**
     * Public success callback invoked by WebAppInterface upon successful parsing.
     */
